...
// Get the result...
if ($result = $mysqli->query('SELECT * FROM employees2 ORDER BY ' .  $column . ' ' . $sort_order)) {
                ...
                <div class="card mt-5">
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <div class="mt-5 mb-3 clearfix">
                                        ...
                                        <a href="createmed.php" class="btn btn-success pull-right"><i class="fa fa-plus"></i> Add New
                                        Product</a>
                                        </div>
                                </div>
                            </div>
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    <th><a href="med.php?column=id&order=<?php echo $asc_or_desc; ?>">ID<i class="fas fa-sort<?php echo $column == 'id' ? '-' . $up_or_down : ''; ?>"></i></a></th>
					                <th><a href="med.php?column=name&order=<?php echo $asc_or_desc; ?>">Medicine Name<i class="fas fa-sort<?php echo $column == 'name' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                    <th>Storage</th>
					                <th><a href="med.php?column=date&order=<?php echo $asc_or_desc; ?>">Expiration Date<i class="fas fa-sort<?php echo $column == 'date' ? '-' . $up_or_down : ''; ?>"></i></a></th>
                                    <th>Days Left</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tr>
                                <?php
                                    ...
                                    $query=$conn->query("select * from `employees2`");

                                ?>
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
					                <td<?php echo $column == 'ID' ? $add_class : ''; ?>><?php echo $row['id']; ?></td>
					                <td<?php echo $column == 'Medicine Name' ? $add_class : ''; ?>><?php echo $row['name']; ?></td>
                                    ...
                                    <?php
                                         ...
                                         if($usedDays==0)
                                        {
                                            $conn->query("update `employees2` set status='Expired' where id='".$row['id']."'");
                                            echo $usedDays;
                                        }
                                        else if ($usedDays<=7){
                                            $conn->query("update `employees2` set status='Expiring Later' where id='".$row['id']."'");
                                            echo $usedDays;

                                        }
                                        else if ($usedDays<=30){
                                            $conn->query("update `employees2` set status='Expiring Soon' where id='".$row['id']."'");
                                            echo $usedDays;
                                    ...
                                    <td><a href="readmed.php?id=<?php echo $row['id']; ?>"><span class="fa fa-eye"></span></a>&nbsp;
                                    <a href="updatemed.php?id=<?php echo $row['id']; ?>"><span class="fa fa-pencil"></span></a>&nbsp;
                                    <a href="deletemed.php?id=<?php echo $row['id']; ?>"><span class="fa fa-trash"></span></a>
                                </td>
				                    </tr>
                                    <?php endwhile; ?>
                            </thead>
                            <tbody>
                                <?php 
                                    ...
                                    if(isset($_GET['search']))
                                    {
                                        ...
                                        $query = "SELECT * FROM employees2 WHERE CONCAT(id,name,address,date) LIKE '%$filtervalues%' ";
